...
def make_dedup_key(symbol, timeframe, strategy, entry, sl, tp):
    """
    Deduplication key: symbol + timeframe + strategy + entry + sl + tp.
    Creates a perfectly stable, unique ID for every trade setup.
    """
    safe_strat = str(strategy).replace(' ', '').replace(':', '').replace('(', '').replace(')', '')
    safe_entry = str(entry).replace(' ', '').replace(':', '')
    safe_sl = str(sl).replace(' ', '').replace(':', '')
    safe_tp = str(tp).replace(' ', '').replace(':', '')
    return f"{symbol}_{timeframe}_{safe_strat}_{safe_entry}_{safe_sl}_{safe_tp}"
    ...
    async with async_playwright() as p:
        ...
        for row in all_rows:
            try:
                ...
                dedup_key = make_dedup_key(symbol_val, tf_val, strat_val, entry_val, sl_val, tp_val)
